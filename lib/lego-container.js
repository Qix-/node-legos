...
function LegoContainer() {
  ...
  this._right.on('end', this.emit.bind(this, 'end'));
...
LegoContainer.prototype.close = function close() {
  this._left.close();
  return this;
};
...
LegoContainer.prototype.deconstruct = function deconstruct() {
  ...
  this._left.removeAllListeners('end');
  if (this.legos.length) {
    this.legos[this.legos.length - 1].unsnap(this._right);
  }

  for (var i = 0, len = this.legos.length; i < len; i++) {
    var lego = this.legos[i];
    lego.removeListener('error', this._error);
  }
